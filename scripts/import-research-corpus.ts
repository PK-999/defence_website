...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const [name, facts] of grouped) {
      ...
      const summary = `${name} is listed for ${first.service}.`;
      const content = facts.map((fact) => `${fact.field}: ${fact.value}${fact.unit ? ` ${fact.unit}` : ""}. ${fact.scope ?? ""}`).join("\n");
      let equipment = await db.equipment.findUnique({ where: { slug } });
      if (!equipment) equipment = await db.equipment.create({ data: { slug, title: name, domain: equipmentDomain(name), category: "Defence equipment", summary, content, status: "Verified primary specification", developmentModel: equipmentModel(name), serviceStatus: /procurement|planned/i.test(name) ? "planned" : "active", originCountries: JSON.stringify(["India"]), specs: JSON.stringify(specs), statusAsOf: first.effectiveDate ?? null, publicationStatus: "DRAFT", contentKind: "EDITORIAL" } });
