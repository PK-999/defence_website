...
**Reconciled:** 8 September 2026. **Current state:** implementation is complete through the candidate-evidence stage of T26, with T27 release documentation and CI in place. T00–T07 and T09–T26 candidate ingest are functional on automated checks; T08 and T26 public publication remain blocked pending provider/accountable-editor inputs, and T28 remains planned pending the next collection scope. Playwright Chromium is installed and the current desktop/mobile browser suite passes.

Final automated pass for this tranche: `npm test` (59 tests across 14 files), `npm run test:components` (1 component test), `npm run test:integration` (14 files/33 tests), `npm run typecheck`, `npm run lint -- --quiet`, `npm run build`, `npm run validate:content`, `npm run validate:database`, and `npm run validate:assets` all passed. `npm run test:e2e` passed all 48 Chromium checks across desktop and mobile projects after installing Playwright's managed Chromium build. The final audit also enforces the canonical comparison `items=` URL, the 100-node/200-edge graph cap, and the 1 MB map-asset budget.
