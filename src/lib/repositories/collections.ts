...
export async function listPublicEntities(type: EntityType, input: CollectionQuery | URLSearchParams | Record<string, string | string[] | undefined>, db: Database): Promise<PageResult<CollectionItem>> {
  ...
  if (type === "Person") {
    ...
    const total = await db.person.count({ where }); const rows = await db.person.findMany({ where, orderBy: [{ title: "asc" }, { id: "asc" }], skip: boundedSkip(total, query), take: query.pageSize, select: { id: true, slug: true, title: true, summary: true, rank: true, serviceBranch: true, year: true, decorations: true, content: true } });
  ...
  if (type === "Conflict") {
    const where = { ...publicFilter, title: textWhere(query.q), dateStart: query.year ? { contains: query.year } : undefined }; const total = await db.conflict.count({ where }); const rows = await db.conflict.findMany({ where, orderBy: [{ title: "asc" }, { id: "asc" }], ...(query.sort === "date" ? {} : { skip: boundedSkip(total, query), take: query.pageSize }), select: { id: true, slug: true, title: true, summary: true, dateStart: true } });
    ...
    return page(ordered.map((row) => item("Conflict", row, [{ label: "Start", value: row.dateStart }])), total, query, []);
  }
  if (type === "Operation") {
    const where = { ...publicFilter, title: textWhere(query.q), category: query.category, dateStart: query.year ? { contains: query.year } : undefined }; const total = await db.operation.count({ where }); const rows = await db.operation.findMany({ where, orderBy: [{ title: "asc" }, { id: "asc" }], ...(query.sort === "date" ? {} : { skip: boundedSkip(total, query), take: query.pageSize }), select: { id: true, slug: true, title: true, summary: true, dateStart: true, category: true } });
    const ordered = query.sort === "date" ? historicalRows(rows).slice(boundedSkip(total, query), boundedSkip(total, query) + query.pageSize) : rows;
    return page(ordered.map((row) => item("Operation", row, [{ label: "Category", value: row.category }, { label: "Start", value: row.dateStart }])), total, query, []);
  }
  if (type === "Unit") {
