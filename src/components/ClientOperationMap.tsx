...
interface Props {
  ...
  /** Render the map immediately without the explanatory location card. */
  compact?: boolean;
  /** Hide the coordinate readout when the map is the only panel content. */
  showActiveCoordinates?: boolean;
}

export function ClientOperationMap({ markers, activeMarkerId, defaultCenter, compact = false, showActiveCoordinates = true }: Props) {
  const [enabled, setEnabled] = React.useState(compact);
  const [retryNonce, setRetryNonce] = React.useState(0);
  return <div className="h-full space-y-3">
    {!compact && <div className="rounded border border-border/60 bg-card/40 p-4 text-sm">
    ...
    {enabled && <div key={retryNonce} className="relative h-full"><DynamicMap markers={markers} activeMarkerId={activeMarkerId} defaultCenter={defaultCenter} showActiveCoordinates={showActiveCoordinates} /><button type="button" onClick={() => setRetryNonce((value) => value + 1)} className="absolute bottom-3 right-3 z-[1000] rounded border border-border bg-background/90 px-2 py-1 text-xs text-muted-foreground hover:text-foreground">Retry map</button></div>}
  </div>;
}
