...
import { isSearchScope } from '@/lib/search/context';
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await searchArchive({ q: query, type: searchParams.get('type') ?? undefined, scope: isSearchScope(rawScope) ? rawScope : undefined, page: Number(searchParams.get('page') ?? "1"), mode: searchParams.get('mode') === "full" ? "full" : "quick" }, prisma);
