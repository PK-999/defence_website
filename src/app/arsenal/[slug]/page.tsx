...
export default async function EquipmentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const facts = [
    ...
    { label: "Service status", value: formatEquipmentValue("serviceStatus", equipment.serviceStatus), evidence: evidenceFor("serviceStatus") },
  ...
  return <>
    <ArticleLayout title={equipment.title} summary={dossier?.overview ?? equipment.summary ?? "Not documented"} content={dossier ? null : readableContent(equipment.content)} facts={facts}>
      ...
      {rows.length > 0 && <section className="mt-12"><h2 className="border-l-2 border-primary pl-4 text-xl font-bold uppercase tracking-wider">Specifications</h2><dl className="mt-5 divide-y divide-border rounded border border-border/60 bg-card/40 px-5">{rows.map((row, index) => {
        ...
        return <div key={`${row.label}-${index}`} className="grid gap-2 py-4 sm:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]"><dt className="text-xs uppercase tracking-wider text-muted-foreground">{row.label}</dt><dd><span className="font-medium">{row.value}{row.unit ? ` ${row.unit}` : ""}</span>{source && <div className="mt-2 space-y-1 text-xs leading-5 text-muted-foreground">{source.canonicalUrl && <p><a href={source.canonicalUrl} target="_blank" rel="noreferrer" className="font-semibold text-primary underline">Source: {source.title}</a>{source.publisher ? ` · ${source.publisher}` : ""}{source.publicationDate ? ` · ${formatDisplayDate(source.publicationDate)}` : ""}</p>}{row.sourceId && !source && <p><span className="font-semibold text-foreground">Source unavailable:</span> {row.sourceId}</p>}</div>}</dd></div>;
      })}</dl></section>}
