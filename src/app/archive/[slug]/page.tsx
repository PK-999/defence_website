...
export default async function SourcePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <PageShell width="wide">
      ...
      <p className="-mt-4 mb-8 text-sm text-muted-foreground">Published by <span className="text-foreground">{source.publisher}</span>{source.publicationDate ? ` · ${formatDisplayDate(source.publicationDate)}` : ""}{source.author ? ` · ${source.author}` : ""}</p>
