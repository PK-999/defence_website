...
export default async function ArchivePage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <PageShell>
      ...
      <form method="get" className="mb-8 grid gap-3 rounded-lg border border-border/60 bg-card/40 p-4 sm:grid-cols-3">
        ...
        <div className="sm:col-span-3 flex items-end gap-3"><button type="submit" className="h-10 rounded bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90">Apply filters</button><Link href="/archive" className="text-sm text-muted-foreground hover:text-foreground">Clear</Link></div>
      </form>
